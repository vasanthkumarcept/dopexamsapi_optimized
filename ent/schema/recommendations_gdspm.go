...
func (RecommendationsGDSPMApplications) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("Post").Optional(),
		field.Bool("Eligible").Optional(),
...
func (RecommendationsGDSPMApplications) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("ApplnRef", Exam_Applications_GDSPM.Type).Ref("GDSPMApplicationsRef").Unique(),
		//.Field("ApplicationID"),
		}
...
func (RecommendationsGDSPMApplications) Annotations() []schema.Annotation {
	return []schema.Annotation{entsql.Annotation{Table: "RecommendationsGDSPMApplications"}}
}
