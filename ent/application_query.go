...
// QueryCenter chains the current query on the "center" edge.
func (aq *ApplicationQuery) QueryCenter() *CenterQuery {
	query := (&CenterClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) QueryNotification() *NotificationQuery {
...
func (aq *ApplicationQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (aq *ApplicationQuery) FirstIDX(ctx context.Context) int32 {
...
func (aq *ApplicationQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (aq *ApplicationQuery) OnlyIDX(ctx context.Context) int32 {
...
func (aq *ApplicationQuery) IDs(ctx context.Context) (ids []int32, err error) {
...
func (aq *ApplicationQuery) IDsX(ctx context.Context) []int32 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withCenter:       aq.withCenter.Clone(),
...
// WithCenter tells the query-builder to eager-load the nodes that are connected to
// the "center" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ApplicationQuery) WithCenter(opts ...func(*CenterQuery)) *ApplicationQuery {
	query := (&CenterClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) WithNotification(opts ...func(*NotificationQuery)) *ApplicationQuery {
	query := (&NotificationClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(application.FieldEmployeeID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Application.Query().
//		Select(application.FieldEmployeeID).
//		Scan(ctx, &v)
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := aq.withCenter; query != nil {
		if err := aq.loadCenter(ctx, query, nodes, nil,
			func(n *Application, e *Center) { n.Edges.Center = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withNotification; query != nil {
		if err := aq.loadNotification(ctx, query, nodes, nil,
			func(n *Application, e *Notification) { n.Edges.Notification = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ApplicationQuery) loadCenter(ctx context.Context, query *CenterQuery, nodes []*Application, init func(*Application), assign func(*Application, *Center)) error {
	ids := make([]int32, 0, len(nodes))
	nodeids := make(map[int32][]*Application)
	for i := range nodes {
...
func (aq *ApplicationQuery) loadNotification(ctx context.Context, query *NotificationQuery, nodes []*Application, init func(*Application), assign func(*Application, *Notification)) error {
	ids := make([]int32, 0, len(nodes))
	nodeids := make(map[int32][]*Application)
	for i := range nodes {
...
func (aq *ApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(application.Table, application.Columns, sqlgraph.NewFieldSpec(application.FieldID, field.TypeInt32))
	_spec.From = aq.sql
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withCenter != nil {
			_spec.Node.AddColumnOnce(application.FieldCenterCode)
		}
		if aq.withNotification != nil {
			_spec.Node.AddColumnOnce(application.FieldNotifyCode)
		}
	}
