...
func (Exam_Applications_IP) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("CACorrected").Optional(),
		field.Time("CACorrectedDate").SchemaType(map[string]string{
			dialect.Postgres: "timestamp",
		}).Optional(),
...
func (Exam_Applications_IP) Indexes() []ent.Index {
	return []ent.Index{
...
func (Exam_Applications_IP) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("ExamCentres", Center.Type). Ref("ExamCentresRef").Unique().Field("CenterCode"),
		edge.From("examcitycenter", ExamCityCenter.Type).Ref("ExamCityCenterRef").Unique().Field("ExamCityCenterCode"),
		edge.From("roleusers", RoleMaster.Type).Ref("Roles_IP_Ref").Unique().Field("RoleUserCode"),
		edge.From("CircleRefs", CircleSummaryForNO.Type).Ref("CircleRefsForHallTicketIP").Unique().Field("CircleSID"),
		//edge.From("ExamCenterHallIPRefs", ExamCenterHall.Type).Ref("ExamCenterHallIPRef").Unique().Field("ExamCenterHall"),
	}
}
